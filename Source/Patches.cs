...
{
    [StaticConstructorOnStartup, HarmonyPatch(typeof(Building_CrashedShipPart), "<TrySpawnMechanoids>m__4A2")]
    public static class Patches
    {
        static Patches()
        {
            HarmonyInstance.Create("com.spdskatr.NoRogueRobots.patches").PatchAll(Assembly.GetExecutingAssembly());
            Log.Message("SS No Rogue Robots initialized. Patched:\nBuilding_CrashedShipPart.<TrySpawnMechanoids>m__4A2 (Non-destructive postfix)\nSymbolResolver_RandomMechanoidGroup.<Resolve>m__271 (Non-destructive postfix)\n\n");
        }
    ...
    [HarmonyPatch(typeof(SymbolResolver_RandomMechanoidGroup), "<Resolve>m__271")]
